...
| 012 | Remove font-semibold from event titles (make thinner) | 1 file: `EventTitle.vue` — remove `font-semibold` class. Event date text already normal weight. | Remove `font-semibold` from the event title div class in EventTitle.vue. Build and verify in browser. | IN PLAN | — | Gate 1 pending. |
